...
int FocusStack::maxLoGIdx(int col, int row, std::vector<Matrix<float>> &log_stack,
    ...
    for (int stack_idx = 0; stack_idx < log_stack.size(); stack_idx++) {
        if (std::fabs(log_stack.at(stack_idx).at(col, row, 0)) > max_laplacian_value &&
            std::fabs(log_stack.at(stack_idx).at(col, row, 0)) > bg_threshold)
        {
            max_laplacian_idx = stack_idx;
            max_laplacian_value = std::fabs(log_stack.at(stack_idx).at(col, row, 0));
        }
    }
